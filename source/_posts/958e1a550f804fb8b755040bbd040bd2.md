...
因此需要实现一个 TransformStream 来将 SSE 文本流转换为结构化的数据，并处理 text 多条消息合并。实现本身并不复杂，但主要的问题是多个模型拆分规则可能规则会不一致，例如 GPT-4o chunk 中可能包含一条完整消息，也可能不包含，而 Claude 3.5 Sonnet 中则总是由两个 chunk 组成一条完整消息。
