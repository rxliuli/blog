...
是的，这里的关键在于 `Response`，它允许接受某种 ReadableStream [^11](https://developer.mozilla.org/docs/Web/API/ReadableStream) 类型的参数，而 ReadableStream 并不保存数据到内存，它只是一个可以不断拉取数据的流。
