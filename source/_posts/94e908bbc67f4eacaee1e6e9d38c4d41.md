---
layout: post
title: Browser Extension Dev - 06. Inject Script on Demand
...
categories:
  ...
  - Browser Extension
date: 1768964241484
updated: 1769342224396
---

## 前言

在上一章 [Browser Extension Dev - 05. Storage and Configuration](/p/3b250074ced142abb1d239d244fc308a) 中，我们介绍了为扩展添加设置页面并使用 Storage API 保存和读取配置的功能。在这一章，我们将介绍按需注入脚本。这种方式完全不会拖慢网站运行速度，同时在 Chrome Web Store 安装时不会有任何安全警告提示。接下来我们将实现一个点击扩展图标快速复制网页中的主要内容为 markdown 的扩展。
...
1. 默认为所有网页注入 Content Script 不仅会拖慢网站速度，还可能导致风险，因为它是完全无感知的，后续扩展更新可能引入漏洞
...
这样，需要的权限就从 `['<all_urls>']` 变成了 `['activeTab', 'scripting']`，尽管数量变多了，但后者的风险相对低很多，必须要用户触发才能执行代码。所以安装扩展时不会有任何警告，例如

![1768996707257.jpg](/resources/07904bc29a334a83bc02cdb59563e594.jpg)

这里涉及到的关键 API 是 `scripting.executeScript`，它的名字很直观，就是为了执行自定义脚本。
...
接下来在后台脚本中实现监听和注入部分。
首先还是更新 wxt.config.ts，添加正确的权限以及空的 action 字段。
...
然后添加一个用于测试的注入脚本，和之前的 Content Script 不太一样，在 WXT 中需要使用 defineUnlistedScript 来声明它。
...
> ⚠️ 局限性：如果你希望点击一次就向指定网站注入脚本并持久化，后续刷新或重新进入页面也能自动运行脚本，这是行不通的，仍然需要正确声明 `host_permissions` 权限来持久化注入 Content Script，即使使用 scripting API 仍然受到权限模型的限制。

### 在注入脚本中实现功能

接下来实现读取网页主要内容，转换为 Markdown，然后复制到剪贴板的功能。在能够使用 npm 生态系统的情况下，这非常简单。
...
在本章中，我们实现了一个按需注入脚本的扩展，它非常轻量，对网页运行没有任何影响，只在需要的时候才运行扩展的代码，即插即用。在下一篇，我们将继续完善这个扩展，使用 Popup 弹窗直接预览和编辑从当前页面复制的 Markdown。
