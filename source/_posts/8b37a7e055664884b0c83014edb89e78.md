...
categories:
  ...
  - nodejs 服务端
date: 1743666994544
updated: 1743677319553
---

## 背景

最近在做一些服务端相关的事情，使用了 Cloudflare Workers 的 D1 数据库，在此过程中，遇到了一些数据库相关的问题，而对于前端而言数据库是一件相当不同的事情，所以在此记录一下。
...
在 update 时应该排除 id（即使实际上没有修改）字段。例如下面的代码，将外部传入的 user 传入并更新，看起来没问题？
...
这导致了吾辈在考虑分页时直接选择了基于 cursor 而非 offset，而且永远不会给出总数，因为即便 id 有索引，统计数量也会扫描所有行。这也是一个已知问题：<https://community.cloudflare.com/t/full-scan-for-simple-count-query/682625>
...
幸运的是，实现一个通用的自动分页器并不麻烦，检查 <https://github.com/drizzle-team/drizzle-orm/issues/2479#issuecomment-2746057769>
