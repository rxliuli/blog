...
![new project.gif](/resource/6ac71ef7d19d409c8e7cef995c396876.gif)
...
1.  选择或输入一些生成器需要的配置
1.  创建项目
1.  如果是当前项目的子目录，打开目录中的 package.json，否则打开一个新的窗口
...
![toolkit-artwork](/resource/4d90e30cfde74c23bef01991a8ea261b.png)
...
![Snipaste_2022-03-29_22-24-32](/resource/8cb9112697704e6699908d0537c647fe.png)
...
1.  确定了通信的基础数据结构
1.  在主线程部分使用 map 保存 `channel => handle` 向渲染线程暴露的功能，在接收到渲染现成的消息后执行相应的方法并通过 postMessage 扔到渲染线程
1.  在渲染线程简单封装，让调用时传入 `channel + data`，即可调用到主线程暴露的方法并异步获取到结果
...
1.  `pnpm create` 功能和 npm 不对等，需要分别适配每个包管理器
1.  webstorm 仍然在使用 npx 实现项目创建，这也影响了吾辈的选择
...
1.  接口不支持 kv 存储，仅支持获取全量状态或设置全量状态
