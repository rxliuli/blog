...
扩展提供了 browser.storage API，允许设置一些 kv 数据，里面可以存储任何可以被[结构化克隆](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm)的数据，通常而言对于扩展的配置功能足够了。除此之外，有时候还使用 localStorage（如果是 Content Script）或 indexeddb（简单的 kv 存储不足以满足需求时）来存储扩展的设置。
其中 browser.storage API 下有几个选项，它们的接口是一致的，只是存储的方式和行为有些不同

- storage.local: 存储在本地的配置，会持久化到本地
...
配置页面是浏览器扩展为扩展提供的一个专用页面，允许在单独的页面中调整扩展的选项，或者访问扩展提供的功能。下面是两种配置页面的使用方式

直接使用浏览器内嵌页面访问，整体上布局比较紧凑，适合配置不多的情况，官方推荐的默认值。
![1768912374888.jpg](/resources/6c644d00f6a74958861fa19016fa65a3.jpg)

或者在单独的标签页访问，作为一个单独的标签页有更大的空间放完整的配置甚至功能，但默认情况下用户无法访问到它，需要调整配置或者编写代码。
...
打开配置页面测试一下没有生效，右键检查控制台发现出现了一个错误。
...
实际上，这意味着缺少 storage 权限。是的，使用任何权限之前都需要声明，快速修改一下 wxt.config.ts。
...
现在，修改页面的中的 Auto Sleep Interval 选项的值之后，刷新页面，可以看到值已经被持久化了。
...
首先在 wxt.config.ts 中的 manifest 中声明 action 选项，目前只要留空就好。
