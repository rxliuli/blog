...
1. 在 [developer.apple.com 的 Certificates 页面](https://developer.apple.com/account/resources/certificates/list) 创建分发证书。Mac App Store 需要两张：**Mac App Distribution**（对应 "3rd Party Mac Developer Application" 签名身份）和 **Mac Installer Distribution**（对应 "3rd Party Mac Developer Installer"，用于给 .pkg 安装包签名）；如果同时发 iOS 版，还需要 **Apple Distribution**（iOS 分发）。创建过程中需要用本机钥匙串生成 CSR（证书签名请求），按页面提示操作即可。
