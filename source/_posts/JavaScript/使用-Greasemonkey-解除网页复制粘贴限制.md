...
可以看出，一般都是使用 JavaScript 在相应事件中返回 false，来阻止对应事件。那么，既然事件都被阻止了，是否意味着我们就束手无策了呢？吾辈所能想到的大致有两种方向

- 使用 JavaScript 监听事件并自行实现复制/剪切/粘贴功能
  - 优点：实现完成后不管是任何网站都能使用，并且不会影响到监听之外的事件
  - 缺点：某些功能自行实现难度很大，例如选择文本
- 重新实现 `addEventListener` 然后删除掉网站自定义的事件
  - 优点：几乎一切都工作得很好
  - 缺点：实现起来需要替换 `addEventListener` 事件**够早**，对某些元素的行内事件不生效
