---
layout: post
title: "JavaScript 避免使用 if-else 的方法\r"
abbrlink: 61dec60eb6664717abf8473d92c7466e
date: 2020-02-02 10:15:37
updated: 2020-12-25 07:07:06
tags:
  - javascript
  - 经验
sticky: null
...
1.  抽取每个状态对应执行的函数
1.  根据状态使用 `if-else/switch` 判断然后调用不同的函数
...
1.  根据不同的状态创建不同的子类，并分别实现基类的方法
1.  添加一个 `Builder` 类，用于根据不同的状态判断来创建不同的子类
1.  调用者使用 `Builder` 类构造出来的对象调用父类中声明的方法
...
1.  创建一个基类，并在其中声明一个需要被子类重写的方法
1.  添加一个 `Builder` 类，具体子类对应的状态由子类的某个属性决定
1.  根据不同的状态创建不同的子类，并分别实现基类的方法，调用 `Builder` 类的方法注册自身
    > 此处因为 js 无法通过反射拿到所有子类，所以子类需要在 `Builder` 类注册自己
1.  使用 `Builder` 构造子类对象，并调用基类声明的方法
